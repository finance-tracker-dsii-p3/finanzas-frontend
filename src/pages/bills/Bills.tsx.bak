...
const Bills: React.FC = () => {
  ...
  const handleDeleteBill = (bill: Bill) => {
    setConfirmModal({
      ...
      message: `¿Estás seguro de que deseas eliminar la factura de ${bill.provider}? Esta acción no se puede deshacer.`,
      type: 'danger',
      onConfirm: async () => {
        try {
                    ...
                    <div>
                      ...
                      <span className={`detail-value ${bill.days_until_due <= 3 && bill.days_until_due >= 0 ? 'text-warning' : ''} ${bill.days_until_due < 0 ? 'text-danger' : ''}`}>
                        {bill.days_until_due > 0
                          ...
                          : bill.days_until_due === 0
                          ? 'Vence hoy'
                          : `Vencida hace ${Math.abs(bill.days_until_due)} días`}
                      </span>
                    </div>
                  </div>
                )}
